...
class Seq2SeqTranslate(object):
    ...
    def __init__(self, emb_words_src, emb_words_tar, vocab_size_src, vocab_size_tar, hidden_size, max_length, layers_num=1, learning_rate=0.1):
        ...
        with tf.name_scope("place_holder"):
            # placeholder
            self.encoder_inputs = tf.placeholder(shape=(None, self.max_length), dtype=tf.int64, name="encoder_inputs")
            self.encoder_length = tf.placeholder(shape=(None, ), dtype=tf.int64, name="encoder_length")
            self.decoder_inputs = tf.placeholder(shape=(None, self.max_length), dtype=tf.int64, name="decoder_inputs")
            self.decoder_target = tf.placeholder(shape=(None, self.max_length), dtype=tf.int64, name="decoder_target")
